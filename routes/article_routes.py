...
def user_can_edit(article):
    return current_user.id == article.author_id or current_user.username in [e.username for e in article.editors or []]
...
def article_detail(article_id):
    ...
    db_sess.commit()
    article.content_html = markdown.markdown(article.content or '', extensions=[
...
def vote_article(article_id):
    ...
    if vote:
        db_sess.delete(vote) if vote.vote == vote_value else setattr(vote, 'vote', vote_value)
    else:
...
def my_articles():
    ...
    db_sess.close()
    return render_template('articles/my_articles.html',
                           articles=articles,
                           article_type_dict=article_type_dict,
...
def article_images(article_id):
    ...
    return render_template('articles/article_images.html',
                           article=article, images=images, form=form)
